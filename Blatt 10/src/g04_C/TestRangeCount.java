...
public class TestRangeCount {
    ...
    /**
     *  Ueberpruefe, ob der Wert, der vom RangeCount-Objekt als Anzahl
     *  der Elemente in [a..b-1] berechnet wird, mit dem angegebenen
     *  Wert uebereinstimmt.
     *  @param rc Zu testendes Objekt
     *  @param a Untere Grenze des Intervals
     *  @param b Obere Grenze des Intervals (exklusive)
     *  @param expect Erwarteter Wert
     *  @return <code>true</code>, falls der erwartete Wert mit 
     *          dem berechneten Wert uebereinstimmt, <code>false</code> sonst.
     */
    private boolean checkExpect(RangeCount rc, Integer a, Integer b, int expect) {
	int count = rc.count(a,b);

	if (count != expect) {
	    System.out.println("Test fehlgeschlagen: rc.count("
			       + a + "," + b + ") = " + count 
			       + " != " + expect);
	    return false;
	}
